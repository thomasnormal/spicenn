...
\begin{figure}[H]
  ...
  \caption{Ngspice DC characterization of the hidden-error replica pair.  Two
  matched diode-loaded MOS forward replicas are nudged by \(+10\) mV and
  \(-10\) mV on the swept preactivation rail, and the finite difference gives the local
  small-signal gain.  The two plotted traces show the positive and negative
  uses of that measured gain for hidden-error transport; this deck validates
  the transistor derivative window rather than inserting a behavioral error
  source.}
