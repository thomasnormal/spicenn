...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\bfseries\Large] at (4.8,3.6) {A. one signed synapse slice};
    \draw[rail] (-0.2,2.8) node[lab,left] {$z^+$} -- (9.8,2.8);
    \draw[rail] (-0.2,1.35) node[lab,left] {$z^-$} -- (9.8,1.35);
    \draw[semithick] (-0.2,0.15) node[lab,left] {$x^+$} -- (9.8,0.15);
    \draw[semithick] (-0.2,-1.05) node[lab,left] {$x^-$} -- (9.8,-1.05);

    \drawnmos{1.2}{1.55}{$M_1$}{$w^+$}{synm1}
    \drawnmos{3.3}{1.55}{$M_2$}{$w^-$}{synm2}
    \drawnmos{6.0}{0.05}{$M_3$}{$w^-$}{synm3}
    \drawnmos{8.1}{0.05}{$M_4$}{$w^+$}{synm4}

    \draw[semithick] (synm1D) |- (1.2,2.8);  \draw[semithick] (synm1S) |- (1.2,0.15);
    \draw[semithick] (synm2D) |- (3.3,2.8);  \draw[semithick] (synm2S) |- (3.3,-1.05);
    \draw[semithick] (synm3D) |- (6.0,1.35); \draw[semithick] (synm3S) |- (6.0,0.15);
    \draw[semithick] (synm4D) |- (8.1,1.35); \draw[semithick] (synm4S) |- (8.1,-1.05);

    \moscap{1.2}{-2.25}{$C_{W^+}$}{syncwp}
    \moscap{3.3}{-2.25}{$C_{W^-}$}{syncwm}
    \draw[gate] (syncwp) -| (synm1G);
    \draw[gate] (syncwp) -| (synm4G);
    \draw[gate] (syncwm) -| (synm2G);
    \draw[gate] (syncwm) -| (synm3G);
    \node[lab, align=center] at (2.45,-3.45)
      {same-sign branches drive \(z^+\); opposite-sign branches drive \(z^-\)};
    \node[group, fit=(synm1D) (synm4S) (syncwp) (syncwm)] {};
  \end{tikzpicture}}
  \caption{Zoomed transistor-level signed synapse slice.  A full local-feature
  cell repeats this MOS pass/transconductance slice for each pixel in the
  receptive field.}
