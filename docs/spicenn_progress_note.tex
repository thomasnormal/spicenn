...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \moscap{2.1}{2.25}{$C_{W^+}$}{wwp}
    \moscap{7.0}{2.25}{$C_{W^-}$}{wwm}
    \draw[semithick] (2.1,3.05) -- (2.1,2.25);
    \draw[semithick] (7.0,3.05) -- (7.0,2.25);

    \drawnmos{0.9}{1.1}{}{$pacc$}{wpaccA}
    \drawnmos{0.9}{0.1}{}{$x^+$}{wxA}
    \drawnmos{0.9}{-0.9}{}{$\delta h^+$}{wdhA}
    \draw[semithick] (wwp) |- (0.9,1.75) -- (wpaccAD);
    \draw[semithick] (wpaccAS) -- (wxAD);
    \draw[semithick] (wxAS) -- (wdhAD);
    \draw[semithick] (wdhAS) -- (0.9,-1.65);
    \mosgnd{0.9}{-1.65}

    \drawnmos{2.9}{1.1}{}{$pacc$}{wpaccB}
    \drawnmos{2.9}{0.1}{}{$x^-$}{wxB}
    \drawnmos{2.9}{-0.9}{}{$\delta h^-$}{wdhB}
    \draw[semithick] (wwp) |- (2.9,1.75) -- (wpaccBD);
    \draw[semithick] (wpaccBS) -- (wxBD);
    \draw[semithick] (wxBS) -- (wdhBD);
    \draw[semithick] (wdhBS) -- (2.9,-1.65);
    \mosgnd{2.9}{-1.65}

    \drawnmos{6.0}{1.1}{}{$pacc$}{wpaccC}
    \drawnmos{6.0}{0.1}{}{$x^+$}{wxC}
    \drawnmos{6.0}{-0.9}{}{$\delta h^-$}{wdhC}
    \draw[semithick] (wwm) |- (6.0,1.75) -- (wpaccCD);
    \draw[semithick] (wpaccCS) -- (wxCD);
    \draw[semithick] (wxCS) -- (wdhCD);
    \draw[semithick] (wdhCS) -- (6.0,-1.65);
    \mosgnd{6.0}{-1.65}

    \drawnmos{8.0}{1.1}{}{$pacc$}{wpaccD}
    \drawnmos{8.0}{0.1}{}{$x^-$}{wxD}
    \drawnmos{8.0}{-0.9}{}{$\delta h^+$}{wdhD}
    \draw[semithick] (wwm) |- (8.0,1.75) -- (wpaccDD);
    \draw[semithick] (wpaccDS) -- (wxDD);
    \draw[semithick] (wxDS) -- (wdhDD);
    \draw[semithick] (wdhDS) -- (8.0,-1.65);
    \mosgnd{8.0}{-1.65}
    \node[lab, align=center] at (1.9,-2.25)
      {\(W^+\) branches\\\(x^+\delta h^+\), \(x^-\delta h^-\)};
    \node[lab, align=center] at (7.0,-2.25)
      {\(W^-\) branches\\\(x^+\delta h^-\), \(x^-\delta h^+\)};
    \node[group, fit=(wwp) (wwm) (wpaccAD) (wdhDS)] {};
  \end{tikzpicture}}
  \caption{Zoomed MOS four-quadrant writer.  Each update path is a stack of
  phase, input, and hidden-error MOS devices.  Same-sign coincidences update
  \(C_{W^+}\); opposite-sign coincidences update \(C_{W^-}\).}
...
\begin{figure}[H]
  ...
  Same-sign input/error coincidence discharges only the \(W^+\) storage
  capacitor in this polarity convention; opposite-sign coincidence discharges
  only \(W^-\).  This validates update-branch selectivity, not yet calibrated
  analog write linearity.}
