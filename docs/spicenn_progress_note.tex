...
\begin{figure}[H]
  ...
  clears that written state without Python forcing the capacitor nodes.}
...
\begin{figure}[H]
  ...
  used by the synapse tail-gate read path.  Same-sign input/error coincidence
  makes a bounded step only on the \(W^+\) storage capacitor; opposite-sign
  coincidence steps only \(W^-\).  The pulse-width sweep verifies near-linear
  selected write magnitude while the inactive branch stays quiet.  The readback
  panel then uses the written \(W^+\) and \(W^-\) voltages as positive- and
  negative-weight synapse tail gates, verifying opposite signed contribution
