...
\begin{figure}[H]
  ...
  activation store.  The top panel is the DC transfer.  The diode-connected
  load is voltage-inverting, so the useful signed load voltage is plotted as
  \(h^- - h^+\).  The bottom panel is a transient storage check: when \(pact\)
  is high the switch tracks the current activation onto \(C_h\), and when
  \(pact\) closes the stored differential activation is held.}
...
\begin{figure}[H]
  ...
  \(-10\) mV on the swept preactivation rail; their finite difference gives the
  local small-signal derivative window.  The bottom panel is a separate
