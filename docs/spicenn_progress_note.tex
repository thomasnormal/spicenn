...
\begin{figure}[H]
  ...
  Same-sign input/error coincidence makes a bounded step only on the \(W^+\)
  storage capacitor; opposite-sign coincidence steps only \(W^-\).  The
  pulse-width sweep verifies that selected write magnitude is near-linear over
  the small update range while the inactive branch stays quiet.}
