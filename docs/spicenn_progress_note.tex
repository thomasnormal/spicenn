...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \drawpmos{2.1}{2.35}{$P_L^+$}{$V_{bp}$}{fp1}
    \drawpmos{6.6}{2.35}{$P_L^-$}{$V_{bp}$}{fp2}
  ...
  \end{tikzpicture}}
  \caption{Zoomed MOS forward cell.  The source-degenerated differential pair
  gives a tanh-like current split, and phase-gated MOS switches store the
  differential activation on \(C_{h^+}\) and \(C_{h^-}\).}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\bfseries\Large] at (4.7,3.8) {C. hidden-error replica pair};
    \draw[semithick] (-0.1,3.15) node[lab,left] {VDD} -- (9.4,3.15);
    \drawpmos{2.1}{2.35}{$P_r^+$}{$r^+$}{bp1}
    \drawpmos{6.6}{2.35}{$P_r^-$}{$r^-$}{bp2}
    \draw[semithick] (bp1S) -- (2.1,3.15); \draw[semithick] (bp2S) -- (6.6,3.15);
    \node[circle, fill=black, inner sep=1.3pt, label=above:{$\delta h^+$}] (bdhp) at (2.1,1.45) {};
    \node[circle, fill=black, inner sep=1.3pt, label=above:{$\delta h^-$}] (bdhm) at (6.6,1.45) {};
    \draw[semithick] (bp1D) -- (bdhp); \draw[semithick] (bp2D) -- (bdhm);
    \drawnmos{2.1}{0.25}{$N_d^+$}{$z^+$}{bn1}
    \drawnmos{6.6}{0.25}{$N_d^-$}{$z^-$}{bn2}
    \draw[semithick] (bdhp) -- (bn1D); \draw[semithick] (bdhm) -- (bn2D);
    \draw[semithick] (bn1S) -- (4.35,-0.65) -- (bn2S);
    \drawnmos{4.35}{-1.6}{$N_{tb}$}{$pbwd$}{bnt}
    \draw[semithick] (4.35,-0.65) -- (bntD); \draw[semithick] (bntS) -- (4.35,-2.45);
    \mosgnd{4.35}{-2.45}
    \moscap{0.55}{0.15}{$C_{\delta h^+}$}{bcdhp}
    \moscap{8.15}{0.15}{$C_{\delta h^-}$}{bcdhm}
    \draw[semithick] (bdhp) -| (bcdhp); \draw[semithick] (bdhm) -| (bcdhm);
    \node[group, fit=(bp1) (bp2) (bn1) (bn2) (bnt) (bcdhp) (bcdhm)] {};
  \end{tikzpicture}}
  \caption{Zoomed MOS hidden-error cell.  The feedback rails \(r^+\) and
  \(r^-\), the derivative-controlled \(z^+/z^-\) pair, and the \(pbwd\) tail
  device form and store the signed hidden-error rails.}
...
\begin{figure}[H]
  ...
  matched MOS forward replicas are nudged by \(+10\) mV and \(-10\) mV on the
  swept preactivation rail, and the finite difference gives the local
  small-signal gain.  The \(r^+\) and \(r^-\) feedback polarities select
  opposite signs, so the cell can transport signed hidden error without a
  behavioral error source.}
