...
four-quadrant MOS coincidence cell: two same-sign branches select the positive
weight capacitor and two opposite-sign branches select the negative weight
capacitor.  Whether the selected branch charges or discharges depends on the
chosen memory polarity; the local requirement is branch selectivity and signed
update direction.
...
\begin{figure}[H]
  ...
  \caption{Ngspice DC characterization of the hidden-error replica pair.  The
  \(r^+\) and \(r^-\) feedback rails produce opposite signs on
  \(\delta h^+ - \delta h^-\), so the cell can transport signed hidden error
  without a behavioral error source.}
