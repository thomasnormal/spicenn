...
def test_phase_transient_sample_source_budget_fails_before_large_decks() -> None:
    phase_transient.validate_sample_source_budget({"sample_source_count": 101}, 0)
    phase_transient.validate_sample_source_budget({"sample_source_count": 100}, 100)

    with pytest.raises(ValueError, match="estimated sample sources"):
        phase_transient.validate_sample_source_budget({"sample_source_count": 101}, 100)
...
def test_phase_transient_hidden_preactivation_source_count_tracks_fusion_mode() -> None:
    assert phase_transient.hidden_preactivation_source_count("node", 16, 2) == 32
    assert phase_transient.hidden_preactivation_source_count("inline", 16, 2) == 0

    with pytest.raises(ValueError, match="hidden_preactivation_mode"):
...
def test_phase_transient_output_rail_source_count_tracks_fusion_mode() -> None:
    assert phase_transient.output_rail_source_count("node", 10) == 10
    assert phase_transient.output_rail_source_count("inline", 10) == 0

    with pytest.raises(ValueError, match="output_rail_mode"):
...
def test_phase_transient_inline_hidden_preactivation_removes_ah_source(tmp_path: Path) -> None:
    ...
    assert "Bpre_h0_0 ah0_0 0 V =" not in netlist
    assert "V(ah0_0)" not in netlist
...
def test_phase_transient_inline_output_rails_remove_y_sources(tmp_path: Path) -> None:
    ...
    assert "By0 y0 0 V =" not in netlist
    assert "By1 y1 0 V =" not in netlist
...
def test_phase_transient_inline_output_rails_cannot_print_y_vectors(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="output_rail_mode=inline"):
